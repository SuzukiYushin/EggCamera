...
const LOCK_NOTE = '\n:lock: ユーザー操作はロック中。撮影→合成→アップの自己診断を実行します。合格すれば自動でユーザー操作を再開、失敗時のみロック継続（`/egg ok` で手動再開）。';
...
const HELP = [
  ...
  '• `help` … この一覧',
  '',
  '再起動系を実行すると、自動で1周セルフテスト（撮影→合成→アップ）を行い結果を通知します。',
  'その間ユーザー操作はロックされ、合格すれば自動再開・失敗時のみロック継続（`/egg ok` で手動再開）。',
